...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template mode="citation" match="citation[lower-case($format) = '?']">
    <textResponse>["<xsl:value-of select="string-join($formats/@key, '&quot;,&quot;')"/>"]
    </textResponse>
  </xsl:template>
  ...
  <xsl:template mode="citation" match="citation[lower-case($format) = 'bibtex']">
    ...
    <textResponse>@data{<xsl:value-of select="uuid"/>,
      ...
      <xsl:if test="lastPublicationDate != ''">year = {<xsl:value-of select="substring(lastPublicationDate, 1, 4)"/>},
      </xsl:if>
      ...
      <div class="row">
        ...
        <div class="col-md-9">
          <p>
            ...
            <div><xsl:copy-of select="translatedTitle/(text()|*)"/>.
            </div>
            ...
            <xsl:if test="additionalCitation != ''">
              <br/>
              <em>
                <xsl:value-of select="$schemaStrings/citationAdditional"/>
              </em>
              <br/>
