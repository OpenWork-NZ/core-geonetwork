...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-citation">
    <xsl:param name="metadata" as="node()"/>
    <xsl:param name="language" as="xs:string"/>

    <xsl:variable name="langId"
                  select="gn-fn-iso19115-3.2018ICSM:getLangId($metadata, $language)"/>
    ...
    <citation>
      ...
      <authorsNameAndOrgList>
        <xsl:copy-of select="$authorsNameAndOrgList"/>
      </authorsNameAndOrgList>
      <lastPublicationDate>
        <xsl:value-of select="$lastPublicationDate"/>
      </lastPublicationDate>
      <translatedTitle>
        <xsl:copy-of select="$translatedTitle"/>
      </translatedTitle>
      <publishersNameAndOrgList>
        <xsl:copy-of select="$publishersNameAndOrgList"/>
      </publishersNameAndOrgList>
      <landingPageUrl>
