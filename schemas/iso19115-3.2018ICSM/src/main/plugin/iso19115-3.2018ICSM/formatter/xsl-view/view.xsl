...
<xsl:stylesheet version="2.0"
  ...
  * render-value taking care of element values (eg. characterString, URL)
  ...
  <xsl:include href="../jsonld/iso19115-3.2018ICSM-to-jsonld.xsl"/>
  ...
  <xsl:variable name="langId" select="gn-fn-iso19115-3.2018ICSM:getLangId($metadata, $language)"/>

  <xsl:variable name="allLanguages">
    <xsl:call-template name="get-iso19115-3.2018ICSM-other-languages"/>
  </xsl:variable>
  ...
  <xsl:template mode="getMetadataTitle" match="mdb:MD_Metadata">
    <xsl:for-each select="if ($isOnlyFeatureCatalog)
      ...
      <xsl:call-template name="get-iso19115-3.2018ICSM-localised">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$language = 'all'">
          ...
          <xsl:variable name="citationInfo">
            <xsl:call-template name="get-iso19115-3.2018ICSM-citation">
  ...
  * only if more than one child to be displayed (non flat mode only) bypass container elements
  ...
  <xsl:template mode="render-field"
    ...
    <div class="gn-contact">
      ...
      <xsl:for-each select="*/cit:party/(cit:CI_Organisation|cit:CI_Individual)">
        ...
        <div class="row">
          <div class="col-md-6">
            <!-- Needs improvements as contact/org are more flexible in iso19115-3.2018ICSM -->
            <address>
              <strong>
                <xsl:choose>
          ...
          <div class="col-md-6">
            <xsl:for-each select=".//cit:contactInfo/*">
              <address>
                <xsl:for-each
                  select="cit:phone/*[cit:numberType/*/@codeListValue = 'voice']/cit:number[normalize-space(.) != '']">
                  <div>
                ...
                <xsl:for-each
                  ...
                  <div>
                    ...
                    <a href="tel:{normalize-space($phoneNumber)}">
                      ...
                      <xsl:comment select="'.'"/>(<xsl:value-of select="../cit:numberType/*/@codeListValue"/>)
                    </a>
                  </div>
                </xsl:for-each>
    ...
    <dl class="gn-link">
      ...
      <dd>
        ...
        <a class="btn btn-link" href="{$nodeUrl}api/records/{$metadataId}/formatters/xml" target="_blank">
          ...
          <span>
            <xsl:value-of select="$schemaStrings/metadataInXML"/>
          </span>
        </a>
      </dd>
    </dl>
  ...
  <xsl:template mode="render-value"
                match="gco:Distance|gco:Measure">
    <span><xsl:value-of select="."/>&#160;<xsl:value-of select="@uom"/>
    </span>
  </xsl:template>
  ...
  <xsl:template mode="render-value"
                ...
                priority="99">
    <div data-gn-field-duration-div="{gco:TM_PeriodDuration}">
      <xsl:value-of select="gco:TM_PeriodDuration"/>
    </div>
  </xsl:template>
  ...
  <xsl:template mode="render-value"
                ...
                priority="100">
    <i class="fa fa-lock text-warning" title="{{{{'withheld' | translate}}}}">
      <xsl:comment select="'.'"/>
    </i>
  </xsl:template>
  <xsl:template mode="render-value"
