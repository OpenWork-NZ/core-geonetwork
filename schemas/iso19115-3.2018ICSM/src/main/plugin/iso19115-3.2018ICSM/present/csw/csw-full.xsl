...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="mdb:MD_Metadata|*[contains(@gco:isoType,'MD_Metadata')]">
    ...
    <xsl:variable name="langId">
      <xsl:call-template name="getLangId19115-3.2018ICSM">
    ...
    <csw:Record>
      ...
      <xsl:for-each select="$identification">
        <xsl:for-each select="mri:citation/cit:CI_Citation/cit:title">
          <dc:title>
            <xsl:apply-templates mode="localised19115-3.2018ICSM" select=".">
        ...
        <xsl:for-each
          select="mri:citation/cit:CI_Citation/cit:date/cit:CI_Date[cit:dateType/cit:CI_DateTypeCode/@codeListValue='revision']/cit:date/*">
          <dct:modified>
            <xsl:value-of select="."/>
          </dct:modified>
        </xsl:for-each>
        ...
        <xsl:for-each
          select="mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue='originator']/cit:party/cit:CI_Organisation/cit:name">
          <dc:creator>
            <xsl:apply-templates mode="localised19115-3.2018ICSM" select=".">
              <xsl:with-param name="langId" select="$langId"/>
            </xsl:apply-templates>
        ...
        <xsl:for-each
          select="mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue='publisher']/cit:party/cit:CI_Organisation/cit:name">
          <dc:publisher>
            <xsl:apply-templates mode="localised19115-3.2018ICSM" select=".">
              <xsl:with-param name="langId" select="$langId"/>
            </xsl:apply-templates>
        ...
        <xsl:for-each
          select="mri:citation/cit:CI_Citation/cit:citedResponsibleParty/cit:CI_Responsibility[cit:role/cit:CI_RoleCode/@codeListValue='author']/cit:party/cit:CI_Organisation/cit:name">
          <dc:contributor>
            <xsl:apply-templates mode="localised19115-3.2018ICSM" select=".">
              <xsl:with-param name="langId" select="$langId"/>
            </xsl:apply-templates>
      ...
      <!-- language -->
      <xsl:for-each
        select="$identification/mri:defaultLocale/lan:PT_Locale/lan:language/lan:LanguageCode/@codeListValue">
        <dc:language>
          <xsl:value-of select="."/>
        </dc:language>
      </xsl:for-each>
      ...
      <xsl:for-each select="../../mdb:parentMetadata">
        <dc:relation>
          <xsl:value-of select="cit:CI_Citation/
          cit:identifier/mcc:MD_Identifier/mcc:code/gco:CharacterString|@uuidref"/>
        </dc:relation>
      </xsl:for-each>
      ...
      <xsl:for-each select="../srv:SV_ServiceIdentification[srv:serviceType/gco:LocalName='OGC:WMS']">
        ...
        <xsl:variable name="connectPoint"
                      select="srv:containsOperations/srv:SV_OperationMetadata/srv:connectPoint/cit:CI_OnlineResource/cit:linkage/*"/>
