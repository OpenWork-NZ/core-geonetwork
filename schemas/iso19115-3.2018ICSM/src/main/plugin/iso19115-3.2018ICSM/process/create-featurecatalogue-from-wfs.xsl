...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="add-featureCat-loc">
    ...
    <msg id="a" xml:lang="fre">Ajouter le catalogue d'attribut à partir du service de téléchargement (WFS ou ESRI:REST)
      :
    </msg>
  </xsl:variable>
  ...
  <xsl:template name="analyze-create-featurecatalogue-from-wfs">
    ...
    <xsl:for-each select="$wfsServices">
      <suggestion process="create-featurecatalogue-from-wfs"
                  ...
                  target="metadata">
        <name>
          <xsl:value-of select="geonet:i18n($add-featureCat-loc, 'a', $guiLang)"/><xsl:value-of select="."/>
        </name>
        ...
        <params>{"featureCatWfsUrl":{"type":"text", "defaultValue":"<xsl:value-of select="substring-before(., '#')"/>"},
          ...
          "featureCatReplace":{"type":"boolean", "defaultValue":"<xsl:value-of select="$featureCatReplace"/>"}}
        </params>
      </suggestion>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template
    ...
    <xsl:copy>
      ...
      <xsl:copy-of select="geonet:make-iso19115-3.2018ICSM-featurecatalogue-from-wfs($featureCatWfsUrl)"/>
  ...
  <xsl:function name="geonet:make-iso19115-3.2018-featurecatalogue-from-wfs" as="node()?">
    ...
    <xsl:choose>
                ...
                <xsl:choose>
                  <xsl:when test="$isEsri">
                    <gfc:featureType>
                      <gfc:FC_FeatureType>
                        <gfc:typeName>
                          <xsl:value-of select="$describeFeatureType/root/name"/>
                        </gfc:typeName>
                        <gfc:isAbstract>
                  ...
                  <xsl:otherwise>
                    <xsl:for-each
                      select="$describeFeatureType//xs:complexType[ends-with(@name, $featureTypeNameNoNamespace) or @name = concat($featureTypeNameNoNamespace, 'Type')]">
                      <gfc:featureType>
                        <gfc:FC_FeatureType>
                          <gfc:typeName>
                            <xsl:value-of select="$featureCatWfsFeatureType"/>
                          </gfc:typeName>
                          <gfc:isAbstract>
                            <gco:Boolean>false</gco:Boolean>
                          </gfc:isAbstract>
                          <xsl:for-each select=".//xs:element">
                            <gfc:carrierOfCharacteristics>
                              <gfc:FC_FeatureAttribute>
                                ...
                                <gfc:cardinality>
                                  <gco:CharacterString>
                                    <xsl:value-of select="if (@minOccurs) then @minOccurs else '1'"/>..<xsl:value-of
                                    select="if (@maxOccurs) then @maxOccurs else '1'"/>
                                  </gco:CharacterString>
                                </gfc:cardinality>
                                <gfc:code>
                                  <gco:CharacterString>
      ...
      <xsl:otherwise>
        <xsl:message>Failed to retrieve DescribeFeatureType document using:
