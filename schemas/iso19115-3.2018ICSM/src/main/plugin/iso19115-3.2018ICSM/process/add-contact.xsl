...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="add-contact-loc">
    ...
    <msg id="a" xml:lang="fre">Ajouter '{{user.name}} {{user.surname}}' en tant que contact pour la fiche ou la
      resource.
    </msg>
  </xsl:variable>
  ...
  <xsl:template name="analyze-add-contact">
    ...
    <xsl:if test="true()">
      <suggestion process="add-contact"
                  ...
                  target="metadata">
        <name>
          <xsl:value-of select="geonet:i18n($add-contact-loc, 'a', $guiLang)"/>
        </name>
        <operational>true</operational>
        <params>{"contactRole":{"type":"codelist", "codelist": "roleCode", "defaultValue": "<xsl:value-of
          select="$contactRole"/>"},
          ...
          "contactSetForResource": {"type": "boolean", "defaultValue": "<xsl:value-of select="$contactSetForResource"/>"}}
        </params>
      </suggestion>
    </xsl:if>
