...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="dateTypes" as="node()+">
    ...
    <status is="completed" dateType="publication" schema="iso19115-3.2018ICSM"/>
    <status is="superseded" dateType="superseded" schema="iso19115-3.2018ICSM"/>
    <status is="deprecated" dateType="deprecated" schema="iso19115-3.2018ICSM"/>
    <status is="obsolete" dateType="deprecated" schema="iso19115-3.2018ICSM"/>
    <status is="retired" dateType="deprecated" schema="iso19115-3.2018ICSM"/>
  </xsl:variable>
    ...
    <xsl:for-each select="$recordStatus">
      ...
      <xsl:if test="$hasNoDateForStatus">
        <suggestion process="add-date-for-status" id="{generate-id()}"
          ...
          <params>{
            "dateTypeForStatus":{"type":"codelist", "codelist": "<xsl:value-of
              select="if ($schema = 'iso19115-3.2018ICSM') then 'cit:CI_DateTypeCode' else 'gmd:CI_DateTypeCode'"/>",
            "defaultValue":"<xsl:value-of select="$dateTypeForStatus"/>"},
