...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
          ...
          <xsl:for-each select="$root//mdb:contentInfo//gfc:featureType/*">
            <suggestion process="add-columns-from-csv" id="{generate-id()}"
              ...
              <params>{
                ...
                "columnListAsCsv":{"type":"textarea",
                "defaultValue":"SHORT_NAME;DESCRIPTION;TYPE;CARDINALITY_MIN..CARDINALITY_MAX"}
                }
              </params>
            </suggestion>
          </xsl:for-each>
  ...
  <xsl:template match="mdb:contentInfo[not($tableExist) and position() = 1]/*/mrc:featureCatalogue[1]/*">
    <xsl:copy>
      <xsl:apply-templates
        select="@*|*[local-name() = 'name' or local-name() = 'scope' or local-name() = 'versionNumber' or local-name() = 'fieldOfApplication' or local-name() = 'characterSet' or local-name() = 'producer' or local-name() = 'locale' or local-name() = 'versionDate' or local-name() = 'functionalLanguage' or local-name() = 'identifier' or local-name() = 'featureType']"/>
      ...
      <xsl:apply-templates
        select="*[local-name() != 'name' and local-name() != 'scope' and local-name() != 'versionNumber' and local-name() != 'fieldOfApplication' and local-name() != 'characterSet' and local-name() != 'producer' and local-name() != 'locale' and local-name() != 'versionDate' and local-name() != 'functionalLanguage' and local-name() != 'identifier' and local-name() != 'featureType']"/>
    </xsl:copy>
  </xsl:template>
