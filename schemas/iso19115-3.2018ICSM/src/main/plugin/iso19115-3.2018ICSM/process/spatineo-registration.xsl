    ...
    <xsl:for-each select="$services">
      ...
      <xsl:if test="not($isRegistered)">
        <suggestion process="spatineo-registration"
                    ...
                    target="metadata">
          <name>
            <xsl:value-of select="geonet:i18n($spatineo-registration-loc, 'a', $guiLang)"/><xsl:value-of select="."/>
          </name>
          <operational>true</operational>
          <params>{"spatineoUrl":{"type":"text", "defaultValue":"<xsl:value-of select="."/>"}}
          </params>
        </suggestion>
      </xsl:if>
    </xsl:for-each>
                    ...
                    <cit:onlineResource>
                      <cit:CI_OnlineResource>
                        <cit:linkage>
                          <gco:CharacterString>
                            <xsl:value-of
                              select="concat('https://directory.spatineo.com/service/', root/wmsServiceID)"/>
                          </gco:CharacterString>
                        </cit:linkage>
                      </cit:CI_OnlineResource>
                    </cit:onlineResource>
