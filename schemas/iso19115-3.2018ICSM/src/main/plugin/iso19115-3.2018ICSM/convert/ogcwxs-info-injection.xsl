    ...
    <xsl:copy>
      ...
      <gco:CharacterString>
        ...
        <xsl:for-each select="$processes">
          *
          <xsl:value-of
            select="concat(local-name(.), ':', ows2:Title, ', ', ows2:Abstract, '(', string-join(wps2:LiteralData/wps2:Format/@mimeType, ', '), ')')"/><xsl:text>
</xsl:text>
        </xsl:for-each>
      </gco:CharacterString>
    </xsl:copy>
  ...
  <xsl:template mode="copy"
    ...
    <xsl:copy>
      ...
      <xsl:if test="$serviceType = 'WMS'">
        <xsl:apply-templates mode="copy"
                             select="mri:graphicOverview[not(contains(*/mcc:fileName/gco:CharacterString, concat('attachments/', $uuid, '.png')))]"/>
      </xsl:if>
            ...
            <xsl:attribute name="codeListValue">
              <xsl:choose>
                <xsl:when test="name(.)='wps:Capabilities' or
                                ...
                                name(.)='wps2:Capabilities'">loosely
                </xsl:when>
  ...
  <xsl:template mode="convert"
    ...
    <mrd:onLine>
      ...
      <cit:CI_OnlineResource>
        <cit:linkage>
          <gco:CharacterString>
            <xsl:value-of select="@xlink:href|@onlineResource"/>
          </gco:CharacterString>
        </cit:linkage>
        <xsl:if test="$isBuildingDatasetRecord">
          <cit:name>
