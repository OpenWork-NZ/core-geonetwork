...
<xsl:stylesheet version="2.0"
  ...
  <!-- A set of templates use to convert thesaurus concept to
       iso19115-3.2018ICSM fragments. -->

  <!-- Convert a concept to an iso19115-3.2018ICSM fragment with an Anchor
        for each keywords pointing to the concept URI-->
  <xsl:template name="to-iso19115-3.2018ICSM-keyword-with-anchor">
    <xsl:call-template name="to-iso19115-3.2018ICSM-keyword">
      <xsl:with-param name="withAnchor" select="true()"/>
    </xsl:call-template>
  </xsl:template>


  <!-- Convert a concept to an iso19115-3.2018ICSM mri:MD_Keywords with an
       XLink which will be resolved by XLink resolver. -->
  <xsl:template name="to-iso19115-3.2018ICSM-keyword-as-xlink">
    <xsl:call-template name="to-iso19115-3.2018ICSM-keyword">
  ...
  <!-- Convert a concept to an iso19115-3.2018ICSM keywords.
    If no keyword is provided, only thesaurus section is adaded.
    -->
  <xsl:template name="to-iso19115-3.2018ICSM-keyword">
    ...
    <xsl:apply-templates mode="to-iso19115-3.2018ICSM-keyword" select=".">
  ...
  <xsl:template mode="to-iso19115-3.2018ICSM-keyword"
    ...
    <mri:descriptiveKeywords>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:call-template name="to-iso19115-3.2018ICSM-md-keywords">
            <xsl:with-param name="withAnchor" select="$withAnchor"/>
            <xsl:with-param name="withThesaurusAnchor" select="$withThesaurusAnchor"/>
            <xsl:with-param name="listOfLanguage" select="$listOfLanguage"/>
            <xsl:with-param name="textgroupOnly" select="$textgroupOnly"/>
  ...
  <xsl:template mode="to-iso19115-3.2018ICSM-keyword" match="*[/root/request/skipdescriptivekeywords]">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>
  ...
  <xsl:template name="to-iso19115-3.2018ICSM-md-keywords">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>
    <mri:MD_Keywords>
                    ...
                    <gcx:Anchor>
                      <xsl:attribute name="xlink:href">
                        <xsl:choose>
                          ...
                          <xsl:otherwise>
                            <xsl:value-of
                              select="concat($serviceUrl, 'api/registries/vocabularies/keyword?thesaurus=', thesaurus/key, '&amp;id=', uri)"/>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:attribute>
      ...
      <xsl:copy-of select="geonet:add-iso19115-3.2018ICSM-thesaurus-info(
  ...
  <xsl:function name="geonet:add-iso19115-3.2018ICSM-thesaurus-info">
  ...
  <!-- Convert a concept to an iso19115-3.2018ICSM extent -->
  <xsl:template name="to-iso19115-3.2018ICSM-extent">
