...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <!-- Relation contained in the metadata record has to be returned
       ...
       or thumbnails
  -->
  <xsl:template mode="relation" match="metadata[mdb:MD_Metadata or *[contains(@gco:isoType, 'MD_Metadata')]]"
                priority="99">
    ...
    <thumbnails>
      <xsl:for-each select="*/descendant::*[name(.) = 'mri:graphicOverview']/*">
        <item>
          ...
          <url>
            <xsl:apply-templates mode="get-iso19115-3.2018ICSM-localized-string"
                                 select="mcc:fileName"/>
          </url>
          <title>
            <xsl:apply-templates mode="get-iso19115-3.2018ICSM-localized-string"
