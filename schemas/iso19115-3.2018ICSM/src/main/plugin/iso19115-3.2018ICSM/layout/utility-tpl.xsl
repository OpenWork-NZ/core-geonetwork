...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-is-service">
    <xsl:value-of
      select="count($metadata/mdb:identificationInfo/srv:SV_ServiceIdentification) > 0"/>
  </xsl:template>

  <xsl:template name="get-iso19115-3.2018ICSM-title">
...
"/>
  ...
  <xsl:template mode="get-formats-as-json" match="mdb:MD_Metadata">
    [
    <xsl:for-each
      select="mdb:distributionInfo/*/mrd:distributionFormat/*/mrd:formatSpecificationCitation/*/cit:title/*/text()">{
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-extents-as-json">[
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-online-source-config">
      ...
      <xsl:for-each select="$metadata/descendant::mrd:onLine[
        ...
        <xsl:if test="$fileName != ''">
          <resource>
            ...
            <title>
              <xsl:value-of select="normalize-space($metadata/mdb:identificationInfo/*/
