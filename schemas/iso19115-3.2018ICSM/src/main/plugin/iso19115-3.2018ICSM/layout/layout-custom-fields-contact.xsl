...
<xsl:stylesheet version="2.0"
  ...
  -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="*[cit:CI_Telephone]" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>

    <xsl:apply-templates mode="mode-iso19115-3.2018ICSM" select="*/cit:*">
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="cit:numberType" priority="2000"/>
      ...
      <div class="col-sm-3 gn-value">
        ...
        <xsl:call-template name="render-codelist-as-select">
          ...
          <xsl:with-param name="name"
                          select="concat(../cit:numberType/cit:CI_TelephoneTypeCode/gn:element/@ref, '_codeListValue')"/>
        </xsl:call-template>
