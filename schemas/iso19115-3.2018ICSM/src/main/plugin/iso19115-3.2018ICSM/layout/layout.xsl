...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM"
    ...
    <xsl:apply-templates mode="mode-iso19115-3.2018ICSM" select="*|@*">
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="gn:*|@gn:*|@*" priority="1000"/>
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="gn:child" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
  ...
  -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="200"
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="200"
    ...
    <xsl:variable name="excluded"
                  select="gn-fn-iso19115-3.2018ICSM:isNotMultilingualField(., $editorConfig)"/>
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="value"
                      select="if ($isMultilingualElement) then $values else *[namespace-uri(.) != $gnUri]"/>
      <xsl:with-param name="errors" select="$errors"/>
      <xsl:with-param name="cls" select="local-name()"/>
  ...
  <!--
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="*|@*" priority="0"/>
-->
  <!-- Codelists -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="200"
                match="*[*/@codeList]">
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="cls" select="local-name()"/>
      <xsl:with-param name="xpath" select="$xpath"/>
      <xsl:with-param name="type" select="gn-fn-iso19115-3.2018ICSM:getCodeListType(name(), $editorConfig)"/>
  ...
  next template -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="400" match="gml:TimeInstantType"/>

  <!-- the gml element having no child eg. gml:name. -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="300" match="gml:*[count(.//gn:element) = 1]">
