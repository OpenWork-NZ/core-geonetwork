...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-language">
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-other-languages-as-json">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:variable name="mainLanguage">
            <xsl:call-template name="get-iso19115-3.2018ICSM-language"/>
          </xsl:variable>
          <xsl:if test="$mainLanguage">
            ...
            <lang>
              <xsl:value-of select="concat('&quot;', $mainLanguage, '&quot;:&quot;#', $mainLanguageId[1], '&quot;')"/>
            </lang>
          </xsl:if>
          ...
          <xsl:for-each select="$metadata/mdb:otherLocale/lan:PT_Locale[
            ...
            <lang>
              <xsl:value-of
                select="concat('&quot;', lan:language/lan:LanguageCode/@codeListValue, '&quot;:&quot;#', @id, '&quot;')"/>
            </lang>
          </xsl:for-each>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template name="get-iso19115-3.2018ICSM-other-languages">
    <xsl:variable name="mainLanguage">
      <xsl:call-template name="get-iso19115-3.2018ICSM-language"/>
    </xsl:variable>
  ...
  <!-- Template used to return a translation if one found,
    ...
    -->
  <xsl:template name="get-iso19115-3.2018ICSM-localised"
                mode="localised"
        ...
        <xsl:choose>
          <xsl:when test="lan:PT_FreeText">
            <xsl:for-each select="lan:PT_FreeText/lan:textGroup/lan:LocalisedCharacterString[. != '']">
              ...
              <div xml:lang="{$metadata//lan:PT_Locale[@id = $id]/lan:language/*/@codeListValue}">
                <xsl:value-of select="."/>
              </div>
            </xsl:for-each>
          </xsl:when>
          <xsl:otherwise>
            <div xml:lang="{$metadata//mdb:defaultLocale/*/lan:language/*/@codeListValue}">
  ...
  <xsl:template name="getLangId19115-3.2018ICSM">
    <xsl:param name="langGui"/>
    <xsl:param name="md"/>

    <xsl:call-template name="getLangIdFromMetadata19115-3.2018ICSM">
  ...
  <!-- Get lang #id in metadata PT_Locale section,  deprecated: if not return the 2 first letters
      ...
      -->
  <xsl:template name="getLangIdFromMetadata19115-3.2018ICSM">
    <xsl:param name="md"/>
    <xsl:param name="lang"/>
