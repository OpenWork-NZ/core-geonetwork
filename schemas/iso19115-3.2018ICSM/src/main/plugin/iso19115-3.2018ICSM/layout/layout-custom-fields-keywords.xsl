...
<xsl:stylesheet version="2.0"
  ...
  <!-- Custom rendering of keyword section
    ...
    * if the thesaurus is available in the catalog, display
    ...
    keywords.
  -->
  <xsl:template mode="mode-iso19115-3.2018ICSM" priority="2000" match="
  ...
  <xsl:template mode="mode-iso19115-3.2018ICSM" match="mri:MD_Keywords" priority="2000">
    <xsl:param name="overrideLabel" select="''" required="no"/>
      ...
      <xsl:when test="$thesaurusConfig">
        ...
        <xsl:variable name="transformations"
                      ...
                      select="if ($thesaurusConfig/@transformations != '')
                              ...
                              else 'to-iso19115-3.2018ICSM-keyword,to-iso19115-3.2018-keyword-with-anchor,to-iso19115-3.2018-keyword-as-xlink'"/>

        <!-- Get current transformation mode based on XML fragement analysis -->
        <xsl:variable name="transformation"
                      select="if (parent::node()/@xlink:href)
                  then 'to-iso19115-3.2018ICSM-keyword-as-xlink'
                  else if (count(mri:keyword/gcx:Anchor) > 0)
                  then 'to-iso19115-3.2018ICSM-keyword-with-anchor'
                  else 'to-iso19115-3.2018ICSM-keyword'"/>
