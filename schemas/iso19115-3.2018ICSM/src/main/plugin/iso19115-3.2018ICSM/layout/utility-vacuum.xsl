...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  * All elements having no child text (ie. normalize-space return '') and
  ...
  The main function call the vacuum-iso19115-3.2018ICSM mode.
  -->
  <xsl:function name="gn-fn-iso19115-3.2018ICSM:vacuum" as="node()">
    <xsl:param name="metadata" as="node()"/>
    <xsl:for-each select="$metadata/*">
      <xsl:apply-templates mode="vacuum-iso19115-3.2018ICSM"
                           select="."/>
    </xsl:for-each>
  </xsl:function>


  <xsl:function name="gn-fn-iso19115-3.2018ICSM:isElementOrChildEmpty"
                as="xs:boolean">
  ...
  <xsl:template mode="vacuum-iso19115-3.2018ICSM"
                match="@*|node()">
    <xsl:variable name="isElementEmpty"
                  select="gn-fn-iso19115-3.2018ICSM:isElementOrChildEmpty(.)"/>
