...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template match="mdb:MD_Metadata">
    <xsl:copy copy-namespaces="no">
      ...
      <xsl:if test="not($isCreationDateAvailable)
        ...
        <mdb:dateInfo>
          <xsl:copy-of select="gn-fn-iso19115-3.2018ICSM:write-date-or-dateTime(/root/env/createDate, 'creation')"/>
        </mdb:dateInfo>
      </xsl:if>
      <xsl:if test="not($isRevisionDateAvailable)">
        <mdb:dateInfo>
          <xsl:copy-of select="gn-fn-iso19115-3.2018ICSM:write-date-or-dateTime(/root/env/changeDate, 'revision')"/>
        </mdb:dateInfo>
      </xsl:if>
        ...
        <xsl:when test="not(mdb:metadataStandard)">
          <mdb:metadataStandard>
            <cit:CI_Citation>
              <cit:title>
                I
                <gco:CharacterString>ISO 19115-1.2018ICSM</gco:CharacterString>
              </cit:title>
            </cit:CI_Citation>
          </mdb:metadataStandard>
        </xsl:when>
        ...
        <mdb:metadataLinkage>
          <cit:CI_OnlineResource>
            <cit:linkage>
              ...
              <gco:CharacterString>
                <xsl:value-of select="$pointOfTruthUrl"/>
              </gco:CharacterString>
            </cit:linkage>
  ...
  <xsl:template match="mac:time|mac:expiryDate|mac:requestedDateOfCollection
    ...
    <xsl:copy>
      <gco:DateTime>
        ...
        <xsl:if test="string-length($value) = 10">T00:00:00</xsl:if>
      </gco:DateTime>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="dqm:*[@codeListValue]" priority="10">
    <xsl:copy>
      ...
      <xsl:attribute name="codeList">
        <xsl:value-of
          select="concat('http://standards.iso.org/iso/19157/resources/Codelists/cat/codelists.xml#',local-name(.))"/>
      </xsl:attribute>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="*[@codeListValue]">
    <xsl:copy>
      ...
      <xsl:attribute name="codeList">
        <xsl:value-of
          select="concat('http://standards.iso.org/iso/19115/resources/Codelists/cat/codelists.xml#',local-name(.))"/>
      </xsl:attribute>
    </xsl:copy>
  </xsl:template>
