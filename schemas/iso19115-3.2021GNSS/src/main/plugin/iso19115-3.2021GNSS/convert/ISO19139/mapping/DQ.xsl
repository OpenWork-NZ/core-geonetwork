<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:template match="gmd:dataQualityInfo" mode="from19139to19115-3.2018GNSS">
          ...
          <xsl:if test="gmd:DQ_DataQuality/gmd:scope">
            <mdq:scope>
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <mcc:MD_Scope>
                    <xsl:apply-templates select="gmd:DQ_DataQuality/gmd:scope/gmd:DQ_Scope/*" mode="from19139to19115-3.2018GNSS"/>
                  </mcc:MD_Scope>
                </xsl:otherwise>
              </xsl:choose>
            </mdq:scope>
              ...
              <xsl:element name="mdq:report">
                ...
                <xsl:element name="{concat('mdq:',$dataQualityReportType)}">
                  <xsl:if test="gmd:nameOfMeasure or gmd:measureIdentification or gmd:measureDescription">
                    ...
                    <mdq:measure>
                      <mdq:DQ_MeasureReference>
                        <xsl:apply-templates select="gmd:measureIdentification" mode="from19139to19115-3.2018GNSS"/>
                  ...
                  <xsl:if
                    ...
                    <mdq:evaluationMethod>
                      <mdq:DQ_FullInspection>
                        ...
                        <mdq:evaluationProcedure>
                          <xsl:apply-templates select="gmd:evaluationProcedure/gmd:CI_Citation" mode="from19139to19115-3.2018GNSS"/>
                        </mdq:evaluationProcedure>
                    ...
                    <xsl:apply-templates select="gmd:result" mode="from19139to19115-3.2018GNSS"/>
                  </xsl:if>
    ...
    <xsl:for-each select="gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage">
      ...
      <xsl:element name="mdb:resourceLineage">
        <mrl:LI_Lineage>
                ...
                <xsl:if test="$dataQualityScopeObject//gmd:MD_ScopeDescription">
                  <mcc:levelDescription>
                    <mcc:MD_ScopeDescription>
                      <xsl:apply-templates select="$dataQualityScopeObject//gmd:MD_ScopeDescription/*" mode="from19139to19115-3.2018GNSS"/>
                      <!--<xsl:call-template name="writeCharacterStringElement">
          ...
          <xsl:apply-templates select="gmd:source" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="gmd:processStep" mode="from19139to19115-3.2018GNSS"/>
        </mrl:LI_Lineage>
      </xsl:element>
    </xsl:for-each>
  </xsl:template>

  <xsl:template match="gmd:report/*/gmd:result/gmd:DQ_QuantitativeResult" mode="from19139to19115-3.2018GNSS">
    <mdq:DQ_QuantitativeResult>
      <xsl:apply-templates select="gmd:value" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:valueUnit" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:valueType" mode="from19139to19115-3.2018GNSS"/>
    </mdq:DQ_QuantitativeResult>
  </xsl:template>

  <!-- added to account for element name change from valueType to valueRecordType - 2014-07-29 -->
  <xsl:template match="gmd:result/gmd:DQ_QuantitativeResult/gmd:valueType" mode="from19139to19115-3.2018GNSS">
    <xsl:element name="mdq:valueRecordType">
      <xsl:apply-templates mode="from19139to19115-3.2018GNSS"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="gmd:report/*/gmd:result/gmd:DQ_ConformanceResult/gmd:specification" mode="from19139to19115-3.2018GNSS">
    <mdq:specification>
  ...
  <xsl:template match="gmd:report/*/gmd:result/gmi:QE_CoverageResult/gmi:resultContentDescription/gmi:MI_CoverageDescription" mode="from19139to19115-3.2018GNSS">
    <xsl:element name="mrc:MI_CoverageDescription">
      <xsl:element name="mrc:attributeDescription">
        <xsl:element name="gco:RecordType">
          <xsl:apply-templates select="./gmd:attributeDescription/gcoold:RecordType/@*" mode="from19139to19115-3.2018GNSS"/>
        </xsl:element>
      </xsl:element>
  ...
  <xsl:template match="gmd:LI_ProcessStep/gmd:source" mode="from19139to19115-3.2018GNSS">
    <mrl:source>
      <xsl:apply-templates select="@*" mode="from19139to19115-3.2018GNSS"/>
      <xsl:if test="*[1]">
        ...
        <xsl:element name="{$lineageElement}">
          ...
          <xsl:apply-templates select="./*/gmd:scaleDenominator" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmd:sourceReferenceSystem" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmd:sourceCitation" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmd:sourceExtent" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmd:sourceStep" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmi:processedLevel" mode="from19139to19115-3.2018GNSS"/>
          <xsl:apply-templates select="./*/gmi:resolution" mode="from19139to19115-3.2018GNSS"/>
        </xsl:element>
      </xsl:if>
    </mrl:source>
  </xsl:template>

  <xsl:template match="gmd:LI_Source/gmd:scaleDenominator | gmi:LE_Source/gmd:scaleDenominator" mode="from19139to19115-3.2018GNSS">
  ...
  <xsl:template match="gmd:LI_Source/gmd:sourceExtent | gmi:LE_Source/gmd:sourceExtent" mode="from19139to19115-3.2018GNSS">
    <mrl:scope>
      <mcc:MD_Scope>
        <mcc:level/>
        <mcc:extent>
  ...
  <xsl:template match="gmi:LE_ProcessStep" mode="from19139to19115-3.2018GNSS">
    <xsl:element name="mrl:LE_ProcessStep">
      <xsl:apply-templates select="@*" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:description" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:dateTime" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:processor" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmd:source" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:processingInformation" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:report" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:output" mode="from19139to19115-3.2018GNSS"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="gmd:LI_ProcessStep/gmd:dateTime | gmi:LE_ProcessStep/gmd:dateTime" mode="from19139to19115-3.2018GNSS" priority="5">
    <mrl:stepDateTime>
      <gml:TimeInstant>
  ...
  <xsl:template match="gmi:LE_Processing" mode="from19139to19115-3.2018GNSS">
    <xsl:element name="mrl:LE_Processing">
      <xsl:apply-templates select="@*" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:algorithm" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:identifier" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:softwareReference" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:procedureDescription" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:documentation" mode="from19139to19115-3.2018GNSS"/>
      <xsl:apply-templates select="gmi:runTimeParameters" mode="from19139to19115-3.2018GNSS"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="gmd:report/gmd:DQ_QuantitativeAttributeAccuracy/gmd:result/gmi:QE_CoverageResult//gmd:attributeDescription" priority="5" mode="from19139to19115-3.2018GNSS"/>
